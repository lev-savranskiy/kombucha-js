...
export class BrushZoomComponent implements OnInit {
    ...
    title = 'Duration by Time of Day Aggregate';
    ...
    private parseData(data: any[]): Session[] {
        return data.map(v => <Session>{startTime: this.parseDate(v.startTime), duration: v.duration / 1000 / 60});
    }
    ...
    private initSvg() {
        ...
        this.area = d3Shape.area()
            ...
            .x((d: any) => this.x(d.startTime))
            .y0(this.height)
            .y1((d: any) => this.y(d.duration));

        this.area2 = d3Shape.area()
            .curve(d3Shape.curveMonotoneX)
            .x((d: any) => this.x2(d.startTime))
            .y0(this.height2)
            .y1((d: any) => this.y2(d.duration));
    ...
    private drawChart(data: Session[]) {

        this.x.domain(d3Array.extent(data, (d: Session) => d.startTime));
        this.y.domain([0, d3Array.max(data, (d: Session) => d.duration)]);
