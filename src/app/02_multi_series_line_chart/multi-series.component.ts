...
export class MultiSeriesComponent implements OnInit {
    ...
    title = 'Duration by Time of Day by Channel';
    private data: object[] = [];
    private devdata: any;
    tdata: any;
    maxValue = 0;
    ...
    private initChart(): void {
        ...
        this.y.domain([
            d3Array.min(this.data, function(c) { return d3Array.min(c['values'], function(d) { return d['value']; }); }),
            d3Array.max(this.data, function(c) { return d3Array.max(c['values'], function(d) { return d['value']; }); })
        ]);
